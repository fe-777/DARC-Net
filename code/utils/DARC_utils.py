...
def update_ema_students_weighted(model1, model2, ema_model, alpha, w1, w2):
    """
    基于权重的 EMA 更新策略
    Args:
        model1: 学生网络 1
        model2: 学生网络 2
        ema_model: 教师网络
        alpha: EMA 衰减率 (如 0.99)
        w1: 学生 1 的权重 (0~1)
        w2: 学生 2 的权重 (0~1)，且 w1+w2=1
    """
    # 获取所有参数的字典
    model1_par = dict(model1.named_parameters())
    model2_par = dict(model2.named_parameters())
    ema_par = dict(ema_model.named_parameters())

    for key in ema_par:
        # 确保参数在 GPU 上且不需要梯度（Teacher 不反向传播）
        param1 = model1_par[key].data
        param2 = model2_par[key].data
        ema_param = ema_par[key].data

        # 1. 混合两个学生的参数： Virtual Student = w1*S1 + w2*S2
        # 注意：这里直接在参数空间进行线性加权
        weighted_student_param = w1 * param1 + w2 * param2
        
        # 2. 标准 EMA 更新： Teacher = alpha * Teacher + (1-alpha) * Virtual_Student
        ema_param.mul_(alpha).add_(weighted_student_param, alpha=1 - alpha)
